...
export function GameSidebar({ items }: GameSidebarProps) {
  return (
    <aside className="space-y-4">
      <section className="surface-border rounded-[20px] bg-panel p-4">
        ...
        <div className="mt-4 space-y-2">
          {items.map((item) => (
            <Link
              className="flex items-center justify-between rounded-xl border border-border bg-background px-3 py-3 text-sm hover:border-accent/60"
